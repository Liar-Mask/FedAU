...
class ResNetPrivate_UL(nn.Module):
    ...
    def forward(self, x):
        ...
        a = self.classifier(out)
        b = self.classifier_ul(out)
        z = torch.cat((a,b),dim=1)

        return z

def resnet_ul(**model_kwargs):
    return ResNetPrivate_UL(BasicPrivateBlock, [2, 2, 2, 2], **model_kwargs)
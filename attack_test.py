...
def get_cmd_arguments():
    ...
    parser.add_argument('--data_ldr_path', 
                        default='/CIS32/zgx/Unlearning/FedUnlearning/log_test_client/ul_samples_whole_client/0.02/alexnet/cifar10/FedUL_dataloader_s4_10_32_0.01_1_2024_1_15.pkl',
                        type=str, help='Path to save or load model checkpoints') 
    parser.add_argument('--model_pkl_path', 
                        default='/CIS32/zgx/Unlearning/FedUnlearning/log_test_client/ul_samples_whole_client/0.02/alexnet/cifar10/Final_s4_iid1_epoch_200_E_2_batch32_lr0.01_c_10_1.0_30.6889_0.8643.pkl',
                        type=str, help='Path to save or load model checkpoints')
    parser.add_argument('--ul_model_pkl_path', 
                        default='/CIS32/zgx/Unlearning/FedUnlearning/log_test_client/ul_samples_whole_client/0.02/alexnet/cifar10/FedUL_model_s4_e199_10_32_0.01_1_2024_1_15_2024_01_15_063723.pkl',
                        type=str, help='Path to save or load model checkpoints')
    
    # /CIS32/zgx/Unlearning/FedUnlearning/log_test_backdoor/amnesiac_ul_samples/0.02/alexnet/cifar10/FedUL_model_s4_e199_10_32_0.01_1_2024_1_11_2024_01_11_160023.pkl
...
def attack_inference(model,
    ...
    # print("true_y:",true_y[0:10])
    # print("pred_y:",pred_y[0:10])
...
def create_attack(dataset, dataPath, modelPath,model_pkl_path,data_ldr_path, ul_model_pkl_path,trainTargetModel, trainShadowModel, need_augm, need_topk, param_init, verbose):
    ...
    print("len(ul_ldr):",len(ul_ldr))
    mia_testX, mia_testY = prepare_attack_data(target_model,ul_ldr,device,top_k)  # 1 1 1 1
    print(len(mia_testX))
    attack_test_acc = attack_inference(attack_model, mia_testX, mia_testY, device) # 0 0 0 0
    print('attack_ul_before_acc:',attack_test_acc)

    ul_after_testX, ul_after_testY = prepare_attack_data(target_model,val_ldr,device,top_k,test_dataset=True)
    attack_test_acc = attack_inference(attack_model, ul_after_testX, ul_after_testY, device)
    print(len(ul_after_testX))
    print('attack_val_before_acc:',attack_test_acc)

    ## ul operation
    ul_model_pkl=torch.load(ul_model_pkl_path)
    ul_state_dict=ul_model_pkl['model_ul_state_dict']
    model_dict=copy.deepcopy(target_model.state_dict())

    alpha=0.9
    weight_ul=((1-alpha)*model_dict['classifier.weight']+alpha*ul_state_dict['classifier_ul.weight'])
    model_dict['classifier.weight']=copy.deepcopy(weight_ul)

    bias_ul=((1-alpha)*model_dict['classifier.bias']+alpha*ul_state_dict['classifier_ul.bias'])
    model_dict['classifier.bias']=copy.deepcopy(bias_ul)

    target_model.load_state_dict(model_dict)

    # test 
    mia_testX, mia_testY = prepare_attack_data(target_model,ul_ldr,device,top_k)
    print(len(mia_testX))
    attack_test_acc = attack_inference(attack_model, mia_testX, mia_testY, device)
    print('attack_ul_after_acc:',attack_test_acc)

    ul_after_testX, ul_after_testY = prepare_attack_data(target_model,val_ldr,device,top_k,test_dataset=True)
    attack_test_acc = attack_inference(attack_model, ul_after_testX, ul_after_testY, device)
    print(len(ul_after_testX))
    print('attack_val_after_acc:',attack_test_acc)
    
   
    mia_testX, mia_testY = prepare_attack_data(target_model,train_ldr,device,top_k)
    attack_test_acc = attack_inference(attack_model, mia_testX, mia_testY, device)
    print(len(mia_testX))
    print('attack_train_ldr_acc:',attack_test_acc)
    

    t_trainX, t_trainY = prepare_attack_data(target_model,t_train_loader,device,top_k)
    attack_test_acc = attack_inference(attack_model, t_trainX, t_trainY, device)
    print(len(t_trainX))
    print('attack_t_train_loader_acc:',attack_test_acc)
...
if __name__ == '__main__':
     ...
     create_attack(dataset= args.dataset,
                   ...
                   ul_model_pkl_path=args.ul_model_pkl_path,
